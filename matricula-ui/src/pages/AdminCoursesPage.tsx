...
import { type Course, listCoursesApi, createCourseApi, updateCourseApi, deleteCourseApi } from "../api/courses.api";
...
export default function AdminCoursesPage() {
  ...
  const save = async () => {
    try {
      ...
      if (editId) await updateCourseApi(editId, nombre.trim());
      else await createCourseApi(nombre.trim());
    ...
    <Container sx={{ mt: 3 }}>
      <Paper sx={{ p: 3 }}>
        ...
        <Stack direction={{ xs: "column", sm: "row" }} spacing={2} sx={{ mb: 2 }}>
          <TextField label="Nombre course" value={nombre} onChange={(e) => setNombre(e.target.value)} fullWidth />

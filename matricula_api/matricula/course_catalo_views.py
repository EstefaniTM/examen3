...
from .mongo_serializers import Course_catalogSerializer

col = db["course_catalog"]
...
def course_catalog_list_create(request):
    ...
    serializer = Course_catalogSerializer(data=request.data)
...
@permission_classes([IsAuthenticated])
def course_catalog_detail(request, id: str):
    _id = oid_or_none(id)
    if _id is None:
    ...
    if request.method in ["PUT", "PATCH"]:
        serializer = Course_catalogSerializer(data=request.data, partial=(request.method == "PATCH"))
        serializer.is_valid(raise_exception=True)

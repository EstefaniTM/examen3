...
from .mongo_serializers import Enrollment_eventSerializer

col = db["enrollment_event"]
...
def enrollment_event_list_create(request):
    if request.method == "GET":
        q = dict(request.query_params)
        docs = [fix_id(d) for d in col.find(q)]
        return Response(docs)

    serializer = Enrollment_eventSerializer(data=request.data)
...
@permission_classes([IsAuthenticated])
def enrollment_event_detail(request, id: str):
    _id = oid_or_none(id)
    if _id is None:
    ...
    if request.method in ["PUT", "PATCH"]:
        serializer = Enrollment_eventSerializer(data=request.data, partial=(request.method == "PATCH"))
        serializer.is_valid(raise_exception=True)
